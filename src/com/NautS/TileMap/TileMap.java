	...
	public void loadTiles(String s) {
		...
		try {
			...
			for(int col = 0; col < numTilesAcross; col ++) {
				...
				tiles[2][col] = new Tile(subimage, Tile.TOPHALF);
				subimage = tileset.getSubimage(col * tileSize, tileSize * 3, tileSize, tileSize);
				tiles[3][col] = new Tile(subimage, Tile.BOTTOMHALF);
			}
		...
		for(int row = rowOffset; row < rowOffset + numRowsToDraw; row++) {
			...
			for(int col = colOffset; col < colOffset + numColsToDraw; col++) {
				...
				g.drawImage(
					...
					(int)y + row * tileSize,
					tileSize, tileSize,
					//map[row][col] * tileSize, map[row][col] * tileSize, (row + 1) * tileSize, tileSize,
					null
				);
			}
		}
